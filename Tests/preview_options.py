...
def render(output, scenario):
    ...
    rects = {key(root): (0, 0, int(root.width), int(root.height))}
    ...
    canvas = Image.new("RGBA", (int(root.width), int(root.height)), (15, 14, 12, 255))
    ...
    result = Image.new("RGB", (int(root.width)+48, int(root.height)+78), (29, 28, 25))
    result.paste(canvas.convert("RGB"), (24, 24))
    draw = ImageDraw.Draw(result)
    draw.text((24, int(root.height)+46), "Offline layout preview / approximate fonts / not an in-game screenshot",
